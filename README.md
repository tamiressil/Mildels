...
def remover_sigla(json_data, sigla_para_remover):
    """ Remove a sigla do JSON somente se ela for um campo chave. """
...
def get_city_data(cidade, uf):
    ...
    # Verifica se o JSON continua com dados após a remoção da sigla
    if not filtrando_demarcation_dict:
        filtrando_demarcation_dict = demarcation_dict  # Se ficou vazio, mantém os dados originais
